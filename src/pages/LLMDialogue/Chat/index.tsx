...
const Chat: FC = () => {
  ...
  const [chatTitle, setChatTitle] = useState(searchParams.get('title') || 'New chat')
  ...
  const handleSend = () => {
    ...
    setTimeout(() => {
      const aiResponse = {
        ...
        role: 'assistant' as const,
        content: '这是一个模拟的AI回复。实际应用中，这里会调用AI API来生成回复。',
        timestamp: new Date(),
      }
        ...
        {loading ? (
          <div className="flex items-center justify-center h-full px-3 sm:px-4 py-4 sm:py-6">
            <div className="text-center text-muted-foreground px-4">
              ...
              <p className="text-base sm:text-lg font-medium">Loading conversation...</p>
            </div>
          </div>
        ) : messages.length === 0 ? (
          <div className="flex items-center justify-center h-full px-3 sm:px-4 py-4 sm:py-6">
            <div className="text-center text-muted-foreground px-4">
              ...
              <p className="text-base sm:text-lg font-medium">Start your conversation</p>
              <p className="text-xs sm:text-sm mt-2">Enter your message to start a conversation with AI</p>
            </div>
          </div>
        ) : (
                ...
                {msg.role === 'user' && (
                  <Avatar className="flex-shrink-0 w-7 h-7 sm:w-8 sm:h-8 order-3">
                    ...
                    <AvatarFallback className="bg-gray-500 text-white text-[10px] sm:text-xs font-medium">
                      {userState.userinfo?.username?.charAt(0).toUpperCase() || 'U'}
                    </AvatarFallback>
                  </Avatar>
                )}
              </div>
