    ...
    pipeline.stages.forEach((stage) => {
      ...
      if (stage.steps && stage.steps.length > 0) {
        ...
        stage.steps.forEach((step) => {
          ...
          if (step.command) {
            lines.push(`        command: "${step.command.replace(/"/g, '\\"')}"`);
          }
          if (step.image) {
...
const yamlToPipeline = (yaml: string, currentPipeline?: Pipeline | null): Pipeline => {
  try {
...
`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(pipeline ? 'Pipeline updated successfully' : 'Pipeline created successfully');
      onOpenChange(false);
    } catch (error: any) {
      toast.error(error.message || (pipeline ? 'Failed to update pipeline' : 'Failed to create pipeline'));
  ...
  const handleStepChange = (stageIndex: number, stepIndex: number, field: string, value: any) => {
    const stages = [...(formData.stages || [])];
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit}>
          <Tabs value={mode} onValueChange={(v) => setMode(v as 'form' | 'yaml')} className="w-full">
                    ...
                    <div key={index} className="flex gap-2 items-center border rounded-md p-3">
                      ...
                      {trigger.type === 'push' || trigger.type === 'pull_request' ? (
                        <Input
                          ...
                          onChange={(e) => handleTriggerChange(index, 'branch', e.target.value)}
                          className="flex-1"
                        />
                        ...
                        <Input
                          ...
                          onChange={(e) => handleTriggerChange(index, 'schedule', e.target.value)}
                          className="flex-1"
                        />
                      ) : null}
                      <Button
              ...
              <div className="space-y-2 border-t pt-4">
                ...
                <div className="space-y-4">
                  {formData.stages?.map((stage, stageIndex) => (
                    <div key={stageIndex} className="border rounded-md p-4 space-y-3">
                      ...
                      <div className="space-y-2 pl-4 border-l-2">
                        ...
                        {stage.steps?.map((step, stepIndex) => (
                          <div key={stepIndex} className="flex gap-2 items-start border rounded-md p-3">
                            <div className="flex-1 space-y-2">
                              <Input
                                placeholder="Step name"
                                value={step.name}
                                onChange={(e) =>
                                  handleStepChange(stageIndex, stepIndex, 'name', e.target.value)
                                }
                              />
                              <select
                                value={step.type}
                                onChange={(e) =>
                                  handleStepChange(stageIndex, stepIndex, 'type', e.target.value)
                                }
                              ...
                              <Input
                                ...
                                value={step.command || ''}
                                onChange={(e) =>
                                  handleStepChange(stageIndex, stepIndex, 'command', e.target.value)
                                }
                              />
                            </div>
                            <Button
            ...
            <TabsContent value="yaml" className="mt-4">
              <div className="space-y-2">
                ...
                <p className="text-xs text-muted-foreground">
                  Edit pipeline configuration in YAML format. Changes will be validated on save.
                </p>
              </div>
            </TabsContent>
          </Tabs>
