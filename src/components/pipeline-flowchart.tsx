...
import { CheckCircle2, XCircle, Clock, Play, ArrowRight, GitBranch, Calendar, User } from 'lucide-react';
        ...
        <div className="relative">
          ...
          <div className="flex items-start gap-6 overflow-x-auto pb-4 px-2">
            {pipeline.stages!.map((stage, stageIndex) => {
              ...
              return (
                <div key={stageIndex} className="flex items-start gap-6 flex-shrink-0">
                  {/* Stage Node */}
                  <div className="flex flex-col items-center min-w-[240px]">
                    ...
                    >
                      ...
                      <div className="flex items-center justify-between mb-3">
                        <div className="flex items-center gap-2">
                          <div className={`rounded-full p-1.5 ${getStatusColor(stageStatus)}`}>
                            {getStatusIcon(stageStatus)}
                          </div>
                          <h4 className="font-semibold text-sm">{stage.name || `Stage ${stageIndex + 1}`}</h4>
                        </div>
                        <Badge variant="outline" className={`text-xs ${getStatusColor(stageStatus)}`}>
                          {stageStatus}
                        </Badge>
                      </div>
                        ...
                        <div className="space-y-2 mt-4 pt-3 border-t border-border/50">
                          {stage.steps.map((step, stepIndex) => {
                            ...
                            return (
                              ...
                              >
                                <div className={`rounded-full p-0.5 flex-shrink-0 ${getStatusColor(stepStatus)}`}>
